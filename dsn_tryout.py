    ...
    while iteration < model.number_of_iterations:
        ...
        if len(replay_memory) >= 3000:
            ...
            for i in range(len(minibatch)):
                random_action = random.random() <= epsilon
                if minibatch[i][4]:
                    y_lst.append(reward_batch[i])
                elif random_action:
                    y_lst.append(reward_batch[i] + model.gamma *
                                 output_1_batch[i][torch.randint(model.number_of_actions, torch.Size([]), dtype=torch.int)])
                else:
                    y_lst.append(reward_batch[i] + model.gamma * output_1_batch[i][action_index])
            y_batch = torch.cat(tuple(y_lst))
        ...
        if iteration % 100000 == 0:
            torch.save(model, "100gap/dsn/current_model_" + str(iteration) + ".pth")
            with open(os.path.join(model.save_folder, "output_dsn.csv"), "w", newline='') as f:
                csv_output = csv.writer(f)
                csv_output.writerow(["iteration", "total_reward"])
                csv_output.writerows(it_rw_lst)
        if iteration % 3000 == 0:
            print( "iteration:", iteration, "elapsed time:", time.time() - start, "epsilon:", epsilon, "action:",
                  action_index.cpu().detach().numpy(), "max_reward:", max_reward, "Q max:",
...
def main(mode):
    ...
    if mode == 'test':
        model = torch.load(
            'replace/image_processing/dsn/current_model_1800000.pth',
